...
- **Evidence:**
  ...
  - `Toplevel::handle_shutdown_requests` returns `ShutdownTimeout` and does not abort the remaining tasks. The maintainer confirms this in upstream issue #103. The caller must abort by hand, which `drain_handlers` does now.
...
- **Acceptance scenarios:** Source and maintainer statements decide all four scenarios. Startup failure: the root subsystem always forwards the error, which gives the same result as the current explicit `match`. Task panic: stop the tree against restart with backoff, a structural difference. Shutdown during blocked admission: the same `select!` on a cancellation token. Bounded cleanup: no automatic abort, and no preemption of blocking threads. The spike compiles no prototype, because no runtime result can change a structural gap.
