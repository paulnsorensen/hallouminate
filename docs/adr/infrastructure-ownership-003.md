...
- **Policy items:** Persistent trip history, status reporting, and external liveness behavior stay unchanged. They belong to `watchdog.rs` and `status.rs`, which the crate does not touch.
