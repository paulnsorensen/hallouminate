...
fn format_no_repo_error(
    ...
    cwd: &Path,
    underlying: &hallouminate_domain::common::HallouminateError,
) -> String {
    ...
    let detail = match underlying {
        hallouminate_domain::common::HallouminateError::Config(msg) => msg.clone(),
...
mod tests {
    ...
    fn format_no_repo_error_surfaces_baseline_path_and_origin_for_xdg() {
        ...
        let err =
            hallouminate_domain::common::HallouminateError::Config("walked from /work".into());
...
"#,
    ...
    fn collect_warnings_is_silent_when_every_top_level_key_is_known() {
        ...
        cfg.corpora.push(hallouminate_domain::common::CorpusConfig {
            name: "docs".into(),
            paths: vec!["/x".into()],
            globs: Vec::new(),
            exclude: Vec::new(),
