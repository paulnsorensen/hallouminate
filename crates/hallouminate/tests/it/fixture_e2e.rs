...
async fn prepare_file_io_errors_propagate_out_of_index_corpus() {
    ...
    use hallouminate_domain::indexer::{DEFAULT_BATCH_SIZE, apply, plan};
    ...
    let disk = hallouminate_domain::corpus::scan(&corpus).expect("scan");
    ...
    > = store
        ...
        .map(|s| {
            (
                hallouminate_domain::common::FileRef::new(std::path::PathBuf::from(&s.file_ref)),
                s,
            )
        })
        .collect();
