...
use hallouminate_domain::ground::{GroundOpts, ground_union};
...
async fn union_ground_preserves_attribution_when_chunk_ids_collide_across_corpora() {
    ...
    .expect("open store");
    let registry =
        hallouminate_domain::indexer::HandlerRegistry::new(text_splitter::Characters, 1500);

    // Index the same directory under two distinct corpus names.
    let corpus_a = hallouminate_domain::common::CorpusConfig {
    ...
    let corpus_b = hallouminate_domain::common::CorpusConfig {
