...
use hallouminate::adapters::sqlite::{apply_schema, open_db};
...
fn switching_embedding_model_refuses_with_reset_hint_and_no_writes() {
    ...
    // Seed the meta row via a plain Connection — sqlite-vec auto-extension is
    // already registered process-wide from the open_db() call above, so the
    // chunks_vec virtual table stays intact across reopens.
    {
        let seed_conn = Connection::open(&db_path).expect("seed connection");
        seed_conn
            .execute(
