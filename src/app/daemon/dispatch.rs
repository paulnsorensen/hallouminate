...
pub async fn dispatch(state: &DaemonState, req: DaemonRequest) -> DaemonResponse {
    ...
    match req.payload {
        ...
        DaemonRequestPayload::ReadMarkdown(req) => handle_read_markdown(&effective, &req_cwd, req).await,
...
async fn handle_read_markdown(cfg: &Config, cwd: &Path, req: ReadMarkdownRequest) -> DaemonResponse {
    let corpora = match effective_corpora(cfg) {
        Ok(v) => v,
        Err(resp) => return resp,
    };
    ...
    let corpus_name = match pick_corpus_or_default(&corpora, &cfg.repositories, cwd, req.corpus.as_deref()) {
        Ok(c) => c.name,
        Err(e) => return DaemonResponse::invalid_params(e.into_inner()),
    };
