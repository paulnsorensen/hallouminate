...
use crate::adapters::sqlite::queries::{
    delete_chunks_for_file, delete_vec_for_chunk, insert_chunk, insert_vec, upsert_file, FileRow,
    NewChunk, NewFile,
};
use crate::domains::common::{CorpusConfig, FileRef, HallouminateError, Mtime, Result};
use crate::domains::corpus::{
...
pub(super) fn purge_vecs_for_file(conn: &Connection, file_id: i64) -> Result<()> {
    let mut stmt = conn.prepare("SELECT chunk_id FROM chunks WHERE file_id = ?1")?;
