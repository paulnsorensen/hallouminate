...
mod tests {
    ...
    fn serialize_response_matches_documented_spec_shape() {
        ...
        let expected = json!({
            ...
            "took_ms": 47,
            "stats": { "fts_hits": 18, "vec_hits": 12, "fused": 22 },
            "docs": {
                "/abs/path/to/file.md": {
    ...
    #[test]
    fn serialize_null_rank_when_modality_missed_chunk() {
        let chunk = DocChunk {
            chunk_id: 7,
            heading_path: vec![],
            line_range: [10, 20],
        ...
        let actual = serde_json::to_value(&chunk).expect("serialize");
        assert_eq!(actual["fts_rank"], json!(null));
        assert_eq!(actual["vec_rank"], json!(2));
    }
