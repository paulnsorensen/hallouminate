...
use crate::domains::common::{ChunkId, Result};

pub fn fts_search(conn: &Connection, query: &str, limit: usize) -> Result<Vec<(ChunkId, f64)>> {
...
mod tests {
    ...
    use crate::adapters::sqlite::queries::{insert_chunk, upsert_file, NewChunk, NewFile};
    ...
    fn seed_file(conn: &Connection, file_ref: &str) -> i64 {
        upsert_file(
            conn,
            &NewFile {
                file_ref,
    ...
    fn seed_chunk(conn: &Connection, file_id: i64, ord: i64, text: &str) -> i64 {
        insert_chunk(
            conn,
            &NewChunk {
                file_id,
